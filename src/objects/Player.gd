...
func debug_stuff():
	...
	if Input.is_action_just_released("DEBUG_disable_snapping"):
		targetDeg = null
	if Input.is_action_just_pressed("DEBUG_disable_camera_rotation"):
		$CameraArea/Camera2D.rotating = false
	elif Input.is_action_just_released("DEBUG_disable_camera_rotation"):
		$CameraArea/Camera2D.rotating = true
